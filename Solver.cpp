...
Solver::Solver(int inputBoard[16][16], int size) {
	boardSize = size;
	moves = 0;
	attempts = 0;
	guessingMode = false;
...
void Solver::getBoard(int outputBoard[16][16]) {
	for (int x = 0; x < 16; x++) {
		for (int y = 0; y < 16; y++) {
			outputBoard[x][y] = board[x][y];
		}
	}
}
...
int Solver::solve() {
	...
	while (true) { // läuft solange bis keine neuen Brücken gefunden wurden
		...
		int islandX = islandCoords[currentIsland][0];
		int islandY = islandCoords[currentIsland][1];
		// Fängt wieder bei der ersten Insel an wenn er einmal durch ist:
		if (currentIsland >= (totalIslands - 1)) currentIsland = 0;
		else currentIsland++;
